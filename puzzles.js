...
let category = args[0];
function readRandomInputFromFile(filePath, category) {
    try {
...
function verticalPadding(lines, numberOfLines) {
...
function horizontalPadding(line, maxWidth) {
    ...
    const clueLine = [getRandomString(leadingPaddingSize), line, getRandomString(trailingPaddingSize)].join('').split('');
...
function createTableRows(lines) {
    const rows = [];
    lines.forEach((line) => {
        rows.push(`<tr>`);
...
function validateSolution(solution) {
    ...
    solution.forEach((line) => {
        ...
        line.solutionCells.forEach((cell) => {
...
let randomSolution = readRandomInputFromFile(WORD_LIST_PATH, category);
let maxAttempts = 10;
while (!validInput) {
...
const formattedLines = lines.map((line) => horizontalPadding(line, MAX_LINE_WIDTH));
const htmlContent = `
<!DOCTYPE html>
<html lang="en">
<head>
...
`;
fs.writeFileSync('puzzle.html', htmlContent);
